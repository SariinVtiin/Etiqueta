...
router.get("/", autenticar, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ sucesso: false, erro: "Erro ao listar tipos de refeição" });
  }
});
...
router.post(
  "/",
  autenticar,
  verificarPermissao("cadastros_refeicoes"),
  async (req, res) => {
    try {
      ...
      if (!nome || nome.trim() === "") {
        return res
          .status(400)
          .json({ sucesso: false, erro: "Nome da refeição é obrigatório" });
      }

      const [existente] = await pool.query(
        "SELECT id FROM tipos_refeicao WHERE nome = ?",
        [nome.trim()],
      );
      if (existente.length > 0) {
        return res
          ...
          .json({
            sucesso: false,
            erro: "Já existe uma refeição com este nome",
          });
      }
      ...
      const [nova] = await pool.query(
        "SELECT * FROM tipos_refeicao WHERE id = ?",
        [resultado.insertId],
      );
      res
        .status(201)
      ...
      res
        .status(500)
        .json({ sucesso: false, erro: "Erro ao criar tipo de refeição" });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      const [existe] = await pool.query(
        "SELECT id FROM tipos_refeicao WHERE id = ?",
        [id],
      );
      if (existe.length === 0)
        return res
          .status(404)
          .json({ sucesso: false, erro: "Refeição não encontrada" });
      ...
      const [duplicado] = await pool.query(
        "SELECT id FROM tipos_refeicao WHERE nome = ? AND id != ?",
        [nome.trim(), id],
      );
      if (duplicado.length > 0)
        return res
          ...
          .json({
            sucesso: false,
            erro: "Já existe outra refeição com este nome",
          });
      ...
      res.json({
        ...
        refeicao: atualizada[0],
      });
    } catch (erro) {
      console.error("Erro ao atualizar refeição:", erro);
      res
        .status(500)
        .json({ sucesso: false, erro: "Erro ao atualizar tipo de refeição" });
    }
  },
);
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      const [existe] = await pool.query(
        "SELECT id, nome FROM tipos_refeicao WHERE id = ?",
        [id],
      );
      if (existe.length === 0)
        return res
      ...
      await pool.query("UPDATE tipos_refeicao SET ativa = ? WHERE id = ?", [
        ativa ? 1 : 0,
        id,
      ]);
      res.json({
        sucesso: true,
        mensagem: `Refeição ${ativa ? "ativada" : "desativada"} com sucesso`,
      });
    } catch (erro) {
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `✅ Lista personalizada ${tem_lista_personalizada ? "ativada" : "desativada"} para refeição ID ${id}`,
      );
      res.json({
        sucesso: true,
      ...
      res
        .status(500)
        .json({ sucesso: false, erro: "Erro ao alterar configuração" });
    }
  },
);
...
router.get("/:id/itens", autenticar, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ sucesso: false, erro: "Erro ao listar itens da refeição" });
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (!req.file) {
        return res
          .status(400)
          .json({ sucesso: false, erro: "Nenhum arquivo enviado" });
      }
      ...
      if (dados.length === 0) {
        return res
          .status(400)
          .json({ sucesso: false, erro: "Arquivo vazio ou sem dados válidos" });
      }

      // Validar se a primeira linha tem a coluna PRODUTO
      const primeiraLinha = dados[0];
      const temProduto =
        "PRODUTO" in primeiraLinha ||
        "produto" in primeiraLinha ||
        "Produto" in primeiraLinha;
      if (!temProduto) {
      ...
      const normalizar = (linha) => ({
        ...
        gramatura:
          linha.GRAMATURA || linha.gramatura || linha.Gramatura || null,
      ...
      if (itensNormalizados.length === 0) {
        return res
          ...
          .json({
            sucesso: false,
            erro: "Nenhum item válido encontrado na planilha",
          });
      }
      ...
      try {
        ...
        console.log(
          `✅ Importação concluída para refeição ${refeicoes[0].nome}: ${itensNormalizados.length} itens`,
        );
...
router.get("/:id/itens/estatisticas", autenticar, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ sucesso: false, erro: "Erro ao buscar estatísticas" });
  }
});

...
// ✅ EXPANDIDO: CRUD completo para gestão de leitos
...
router.get('/:id', async (req, res) => {
  try {
    const [leitos] = await pool.query('SELECT * FROM leitos WHERE id = ?', [req.params.id]);
    if (leitos.length === 0) {
      return res.status(404).json({ sucesso: false, erro: 'Leito não encontrado' });
    }
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!numero || !setor) {
      return res.status(400).json({ sucesso: false, erro: 'Número e setor são obrigatórios' });
    }
    ...
    res.status(201).json({
      ...
      leito: { id: resultado.insertId, numero, setor: setor.toUpperCase(), andar, ativo: true }
    });
  } catch (erro) {
...
router.patch('/:id/toggle', async (req, res) => {
  try {
    ...
    await pool.query('UPDATE leitos SET ativo = ? WHERE id = ?', [ativo, req.params.id]);

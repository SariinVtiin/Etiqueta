...
router.get(
  "/exportar",
  autenticar,
  verificarPermissao("cadastros_logs"),
  async (req, res) => {
    try {
      ...
      const fontResumoLabel = {
        name: "Arial",
        size: 10,
        bold: true,
        color: { argb: "FF334155" },
      };
      const fontResumoValor = {
        name: "Arial",
        size: 11,
        bold: true,
        color: { argb: "FF0D9488" },
      ...
      if (tipoEvento)
        textoFiltros += ` | Tipo: ${traduzirTipoEvento(tipoEvento)}`;
      ...
      adicionarLinhaResumo(
        ...
        estatisticas.usuariosUnicosLogados,
        fontResumoLabel,
        fontResumoValor,
        CORES.resumoBg,
      );
      ...
      if (estatisticas.ipsSuspeitos && estatisticas.ipsSuspeitos.length > 0) {
        ...
        for (const ip of estatisticas.ipsSuspeitos) {
          ...
          sheet.getCell(`B${linhaAtual}`).font = {
            name: "Arial",
            size: 9,
            color: { argb: "FFDC2626" },
          };
      ...
      const colunas = [
        ...
        {
          header: "Navegador/Dispositivo",
          key: "user_agent_resumido",
          width: 30,
        },
      ];
      ...
      console.log(
        `[LOG-LOGIN] Relatório Excel gerado: ${logs.length} registros | ${nomeArquivo}`,
      );
    } catch (erro) {
...
function adicionarLinhaResumo(
  ...
  if (corBg) {
    sheet.getCell(`A${linha}`).fill = {
      type: "pattern",
      pattern: "solid",
      fgColor: { argb: corBg },
    };
    sheet.getCell(`B${linha}`).fill = {
      type: "pattern",
      pattern: "solid",
      fgColor: { argb: corBg },
    };

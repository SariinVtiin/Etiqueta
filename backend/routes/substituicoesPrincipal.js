...
router.post(
  "/categorias",
  autenticar,
  verificarPermissao("cadastros_substituicao_principal"),
  async (req, res) => {
    try {
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `✅ Categoria ${ativa ? "ativada" : "desativada"}: ${existe[0].nome}`,
      );
  ...
  async (req, res) => {
    try {
      ...
      if (!categoria_id || !nome || nome.trim() === "") {
        return res
          .status(400)
          .json({ sucesso: false, erro: "Categoria e nome são obrigatórios" });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (itemAtual.length === 0) {
        return res
          .status(404)
          .json({ sucesso: false, erro: "Item não encontrado" });
      }
      ...
      if (duplicado.length > 0) {
        return res
          ...
          .json({
            sucesso: false,
            erro: "Já existe outro item com este nome nesta categoria",
          });
      }
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `✅ Item ${ativo ? "ativado" : "desativado"}: ${existe[0].categoria_nome} ${existe[0].nome}`,
      );

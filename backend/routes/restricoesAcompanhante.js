...
router.get("/", autenticar, async (req, res) => {
  try {
    ...
    console.error(
      "❌ Erro ao listar condições nutricionais do acompanhante:",
      erro,
    );
    res.status(500).json({
      sucesso: false,
      erro: "Erro ao buscar condições nutricionais do acompanhante",
    });
  }
});
...
router.post(
  "/",
  autenticar,
  verificarPermissao("cadastros_condicoes_acompanhante"),
  async (req, res) => {
    try {
      ...
      const [nova] = await pool.query(
        "SELECT * FROM condicoes_nutricionais_acompanhante WHERE id = ?",
        [resultado.insertId],
      );

      console.log(
        `✅ condição nutricional acompanhante criada: ${nome} (ID: ${resultado.insertId})`,
      );
      ...
      console.error(
        "❌ Erro ao criar condição nutricional do acompanhante:",
        erro,
      );
      res
        .status(500)
        .json({ sucesso: false, erro: "Erro ao criar condição nutricional" });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      const [existe] = await pool.query(
        "SELECT id FROM condicoes_nutricionais_acompanhante WHERE id = ?",
        [id],
      );
      if (existe.length === 0) {
      ...
      console.log(
        `✅ condição nutricional acompanhante atualizada: ${nome} (ID: ${id})`,
      );
      ...
      console.error(
        "❌ Erro ao atualizar condição nutricional do acompanhante:",
        erro,
      );
      res.status(500).json({
        sucesso: false,
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      const [existe] = await pool.query(
        "SELECT id, nome FROM condicoes_nutricionais_acompanhante WHERE id = ?",
        [id],
      );
      if (existe.length === 0) {
        return res.status(404).json({
      ...
      await pool.query(
        "UPDATE condicoes_nutricionais_acompanhante SET ativa = ? WHERE id = ?",
        [ativa ? 1 : 0, id],
      );

      console.log(
        `✅ Condição nutricional acompanhante ${ativa ? "ativada" : "desativada"}: ${existe[0].nome}`,
      );
